...
def tokenize_and_postprocess_data(prompt: Union[str, list[str]],
    ...
    rattention_mask = []
    for pt in prompt:
        input_data = tokenizer(pt, return_tensors='pt', add_special_tokens=False)
        ...
        rinput_id.append(input_ids)
        rattention_mask.append(attention_mask)
    return torch.cat(rinput_id, dim=0), torch.cat(rattention_mask, dim=0)
