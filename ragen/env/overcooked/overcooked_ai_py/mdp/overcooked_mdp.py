...
class OvercookedGridworld(object):
    ...
    def get_state_transition(self, state, joint_action):
        ...
        sparse_reward, shaped_reward, sparse_reward_by_agent, shaped_reward_by_agent = self.resolve_interacts(new_state, joint_action)
        ...
        return new_state, sparse_reward, shaped_reward, sparse_reward_by_agent, shaped_reward_by_agent
        ...
        for _, player, action in enumerate(zip(new_state.players, joint_action)):
            ...
            if terrain_type == 'X':
                ...
                if len(nearly_ready_pots) > dishes_already and len(dishes_on_counters) == 0:
                    ...
                    shaped_reward_by_agent[_] += self.reward_shaping_params["DISH_PICKUP_REWARD"]
                ...
                if player.get_object().name == 'dish' and new_state.has_object(i_pos):
                    ...
                    if num_items == self.num_items_for_soup and cook_time >= self.soup_cooking_time:
                        ...
                        shaped_reward_by_agent[_] += self.reward_shaping_params["SOUP_PICKUP_REWARD"]
                    ...
                    if not new_state.has_object(i_pos):
                        ...
                        shaped_reward_by_agent[_] += self.reward_shaping_params["PLACEMENT_IN_POT_REW"]
                ...
                if obj.name == 'soup':
                    ...
                    sparse_reward_by_agent[_] += delivery_rew                        
                ...
                if self.is_gate_closed(i_pos):
                    ...
                    shaped_reward_by_agent[_] += self.reward_shaping_params["GATE_OPENING_REWARD"]

                    # shaped_reward += self.reward_shaping_params["GATE_OPENING_REWARD"]
        assert(sparse_reward == sum(sparse_reward_by_agent))
        assert(shaped_reward == sum(shaped_reward_by_agent))
        return sparse_reward, shaped_reward, sparse_reward_by_agent, shaped_reward_by_agent
