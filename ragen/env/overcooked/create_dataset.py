...
Conventions = f'''1. We want to be efficient and prepare for the next soup while the current soup is cooking. 
'''
Base_prompt = """I am {player_name}. I am playing the game Overcooked with my partner {other_player_name}. {envDescription} """ + f'''
Overcooked has the following rules: {Rules}. We have agreed to follow the following conventions: {Conventions}. I'll provide my action history, current state, teammate's status, and my possible actions. Help me select the best action from the list. Format your response as: Action: <action>. Only select one action. Do not say anything else. Got it?'''

Assistant_response_initial = f'''Got it!'''
...
def main():
    ...
    for seed in seeds:
        ...
        Message_Alice = [
                    ...
                    {"role": "user", "content": Base_prompt.format(player_name="Alice", other_player_name="Bob", envDescription=EnvDescriptions[layout_name])},
                    {"role": "assistant", "content": Assistant_response_initial},
                    {"role": "user", "content": tokenizer.apply_chat_template(env.am[0].prepare_next_move(env.state), add_generation_prompt=True, tokenize=False)},
                ]
        Message_Bob = [
                    {"role": "system", "content": LLM_system_prompt},
                    {"role": "user", "content": Base_prompt.format(player_name="Bob", other_player_name="Alice", envDescription=EnvDescriptions[layout_name])},
                    {"role": "assistant", "content": Assistant_response_initial},
                    {"role": "user", "content": tokenizer.apply_chat_template(env.am[1].prepare_next_move(env.state), add_generation_prompt=True, tokenize=False)},
                ]
        instructions.append([
